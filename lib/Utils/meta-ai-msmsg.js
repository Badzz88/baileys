...
const decryptMsmsgBotMessage = async (messageSecret, messageKey, msMsg) => {
	...
	const baseKey = Buffer.from(Crypto_1.hkdf(toBuffer(messageSecret), KEY_LENGTH, { info: BOT_MESSAGE_INFO }))
	...
	for (const msgId of msgIdCandidates) {
		...
		for (const targetJid of targetJidCandidates) {
			...
			const key = Buffer.from(Crypto_1.hkdf(baseKey, KEY_LENGTH, { info }))
			const aad = Buffer.concat([idBuf, Buffer.from([0x00]), botJidBuf])
			try {
				return Buffer.from(Crypto_1.aesDecryptGCM(payload, key, iv, aad))
			} catch (e) {
				lastError = e
			}
		}
