...
// Pure-JS fallback encoder, used whenever the native bridge is unavailable
// or fails on a given node -- mirrors the algorithm WhatsApp Web itself
// uses (same one Baileys shipped before the native bridge existed).
...
const encodeBinaryNode = node => {
	if (!rb) {
		return encodeBinaryNodeJS(node)
	}
	try {
		return Buffer.from(rb.encodeNode(node))
	} catch (_) {
		return encodeBinaryNodeJS(node)
	}
}
